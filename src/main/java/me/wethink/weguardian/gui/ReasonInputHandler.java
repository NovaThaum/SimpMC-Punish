...
implements Listener {
    ...
    public ReasonInputHandler(WeGuardian plugin, Player staff, OfflinePlayer target, PunishmentType type, long durationMs) {
        this.plugin = plugin;
        this.staff = staff;
        this.target = target;
        this.type = type;
    ...
    private void executePunishment(String reason) {
        ...
        CompletableFuture<?> future = switch (this.type) {
            ...
            case BANIP, TEMPBANIP, MUTEIP, TEMPMUTEIP -> throw new UnsupportedOperationException("IP-based punishments must be executed via commands, not the GUI");
        };
    ...
    private void broadcastToStaff(String message) {
        ...
        for (Player p : players) {
            if (!p.hasPermission("weguardian.staff")) continue;
