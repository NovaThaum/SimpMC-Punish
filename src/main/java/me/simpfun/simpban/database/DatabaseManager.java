 ...
 */
package me.simpfun.simpban.database;
...
public class DatabaseManager {
    ...
    public DatabaseManager(SimpBan plugin) {
        this.plugin = plugin;
    }

    public void initialize() {
        this.databaseType = this.plugin.getConfig().getString("database.type", "sqlite").toLowerCase();
        this.plugin.getLogger().info("Initializing database type: " + this.databaseType);
        try {
            this.dataSource = new HikariDataSource(switch (this.databaseType) {
                ...
                default -> {
                    this.plugin.getLogger().warning("Unknown database type '" + this.databaseType + "', defaulting to SQLite");
                    this.databaseType = "sqlite";
                    yield this.initializeSQLite();
                }
            });
            this.plugin.getLogger().info("Database connection pool initialized successfully (" + this.databaseType.toUpperCase() + ")");
    ...
    private HikariConfig initializeSQLite() {
        ...
        config.setPoolName("SimpBan-SQLite-Pool");
    ...
    private HikariConfig initializeMySQL() {
        ...
        String database = this.plugin.getConfig().getString("database.mysql.database", "simpban");
        ...
        config.setPoolName("SimpBan-MySQL-Pool");
    ...
    private void initializeTables() {
        ...
        ((CompletableFuture)((CompletableFuture)this.executeAsync(createPunishmentsTable).thenCompose(v -> this.executeAsync(createPlayerIpsTable))).thenAccept(v -> {
            ...
            this.plugin.getLogger().log(Level.SEVERE, "Failed to initialize database tables", (Throwable)e);
            return null;
        });
    }
    ...
    private void createIndexSafe(String indexName, String tableName, String columns) {
        ...
        try (Connection conn = this.getConnection();
            ...
            this.plugin.getLogger().log(Level.WARNING, "Failed to create index " + indexName + ": " + e.getMessage());
        }
    }
