 ...
 */
package me.simpfun.simpban.manager;
...
public class PunishmentManager {
    ...
    private void startExpiryTask() {
        ...
        this.plugin.getSchedulerManager().runAsyncRepeating(() -> this.punishmentDAO.cleanupExpired().thenAccept(count -> {
            if (count > 0) {
                this.plugin.getLogger().info("Cleaned up " + count + " expired punishments.");
    ...
    public CompletableFuture<Boolean> unban(UUID targetUUID, UUID staffUUID, String staffName, String reason) {
        ...
        return this.punishmentDAO.deactivate(targetUUID, staffUUID, staffName, reason != null ? reason : "Unbanned", PunishmentType.BAN, PunishmentType.TEMPBAN).thenApply(count -> {
            if (count > 0) {
    ...
    public CompletableFuture<Boolean> unmute(UUID targetUUID, UUID staffUUID, String staffName, String reason) {
        String targetName = Bukkit.getOfflinePlayer((UUID)targetUUID).getName();
        return this.punishmentDAO.deactivate(targetUUID, staffUUID, staffName, reason != null ? reason : "Unmuted", PunishmentType.MUTE, PunishmentType.TEMPMUTE).thenApply(count -> {
            if (count > 0) {
                this.cacheManager.invalidateMute(targetUUID);
    ...
    private void notifyMute(UUID targetUUID, String playerName, String staffName, String reason, Instant expiresAt) {
        Player target = Bukkit.getPlayer((UUID)targetUUID);
        if (target != null && target.isOnline()) {
            String muteMsg = this.msg().getMessage("punishments.mute.applied", "{reason}", reason != null ? reason : "No reason specified", "{expires}", TimeUtil.formatRemaining(expiresAt), "{player}", playerName != null ? playerName : "Unknown", "{staff}", staffName != null ? staffName : "Console");
    ...
    public String buildKickMessage(String playerName, String staffName, PunishmentType type, String reason, Instant expiresAt) {
        ...
        return template.replace("{reason}", reason != null ? reason : "No reason specified").replace("{expires}", TimeUtil.formatRemaining(expiresAt)).replace("{player}", playerName != null ? playerName : "Unknown").replace("{staff}", staffName != null ? staffName : "Console");
    }
}
