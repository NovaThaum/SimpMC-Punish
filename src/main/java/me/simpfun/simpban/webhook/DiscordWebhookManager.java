 ...
 */
package me.simpfun.simpban.webhook;
...
public class DiscordWebhookManager {
    ...
    public DiscordWebhookManager(SimpBan plugin) {
        this.plugin = plugin;
    ...
    public void reload() {
        ...
        if (this.enabled && (this.webhookUrl == null || this.webhookUrl.isEmpty() || this.webhookUrl.equals("YOUR_WEBHOOK_URL_HERE"))) {
            this.plugin.getLogger().warning("Discord webhook is enabled but no valid URL is configured!");
            this.enabled = false;
        }
        if (this.enabled) {
            this.plugin.getLogger().info("Discord webhook logging enabled.");
        }
    }
    ...
    public void logUnban(String targetName, String staffName) {
        ...
        WebhookEmbed embed = new WebhookEmbed("\ud83d\udd13 Player Unbanned", String.format("**%s** has been unbanned by **%s**", targetName, staffName), 65416, Instant.now());
        this.pendingEmbeds.offer(embed);
    }
    ...
    public void logUnmute(String targetName, String staffName) {
        ...
        WebhookEmbed embed = new WebhookEmbed("\ud83d\udd0a Player Unmuted", String.format("**%s** has been unmuted by **%s**", targetName, staffName), 0x88FF00, Instant.now());
        this.pendingEmbeds.offer(embed);
    }
    ...
    public void logUnbanIp(String ipAddress, String targetName, String staffName) {
        ...
        WebhookEmbed embed = new WebhookEmbed("\ud83d\udd13 IP Unbanned", String.format("IP `%s` (**%s**) has been unbanned by **%s**", ipAddress, targetName, staffName), 65416, Instant.now());
        this.pendingEmbeds.offer(embed);
    }
    ...
    public void logUnmuteIp(String ipAddress, String targetName, String staffName) {
        ...
        WebhookEmbed embed = new WebhookEmbed("\ud83d\udd0a IP Unmuted", String.format("IP `%s` (**%s**) has been unmuted by **%s**", ipAddress, targetName, staffName), 0x88FF00, Instant.now());
        this.pendingEmbeds.offer(embed);
    }
    ...
    private WebhookEmbed createPunishmentEmbed(Punishment punishment) {
        ...
        description.append("**Reason:** ").append(punishment.getReason() != null ? punishment.getReason() : "No reason specified").append("\n");
        ...
        if (punishment.getType().isTemporary() && punishment.getExpiresAt() != null) {
            description.append("**Duration:** ").append(TimeUtil.formatRemaining(punishment.getExpiresAt())).append("\n");
    ...
    private void sendWebhook(String json) throws IOException {
        ...
        try {
            ...
            connection.setRequestProperty("User-Agent", "SimpBan/" + this.plugin.getPluginMeta().getVersion());
            ...
            if (responseCode == 429) {
                this.plugin.getLogger().warning("Discord webhook rate limited. Consider increasing batch interval.");
