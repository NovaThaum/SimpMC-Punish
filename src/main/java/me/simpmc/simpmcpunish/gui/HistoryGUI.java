...
extends FastInv {
    ...
    public static void initializeIcons() {
        MessagesManager msg = SimpMCPunish.getInstance().getMessagesManager();
    ...
    public HistoryGUI(SimpMCPunish plugin, Player staff, OfflinePlayer target, List<Punishment> history) {
        this(plugin, staff, target, history, 0);
    }

    public HistoryGUI(SimpMCPunish plugin, Player staff, OfflinePlayer target, List<Punishment> history, int page) {
    ...
    private ItemStack createPunishmentItem(Punishment punishment) {
        ...
        String status = punishment.isActive() ? (punishment.isExpired() ? "已过期" : "生效中") : "已移除";
        ...
        lore.add("");
        lore.add(MessageUtil.colorize("&7类型: " + punishment.getType().getColor() + punishment.getType().getDisplayName()));
        lore.add(MessageUtil.colorize("&7原因: &f" + (punishment.getReason() != null ? punishment.getReason() : "无")));
        lore.add(MessageUtil.colorize("&7执行者: &f" + punishment.getStaffName()));
        lore.add(MessageUtil.colorize("&7时间: &f" + TimeUtil.formatDate(punishment.getCreatedAt())));
        lore.add("");
        lore.add(MessageUtil.colorize("&7时长: &f" + durationText));
        lore.add(MessageUtil.colorize("&7状态: " + statusColor + status));
        if (!punishment.isActive() && punishment.getRemovedByName() != null) {
            lore.add("");
            lore.add(MessageUtil.colorize("&7移除者: &f" + punishment.getRemovedByName()));
            lore.add(MessageUtil.colorize("&7移除时间: &f" + TimeUtil.formatDate(punishment.getRemovedAt())));
        }
    ...
    public static void openAsync(SimpMCPunish plugin, Player staff, OfflinePlayer target) {
